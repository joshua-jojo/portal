<thead>
				...
				<tbody>
					<?php
						...
						$querydosen = mysqli_query ($konek, " select DISTINCT id_presensi,nama,kode_kelas,jadwal.id_pelajaran,nama_kelas, nama_pelajaran,DATE_FORMAT(tanggal,'%d-%m-%Y') as tanggal,statuss from presensi inner join jadwal on presensi.id_pelajaran = jadwal.id_pelajaran inner join pelajaran on presensi.id_pelajaran = pelajaran.kode_pelajaran inner join kelas on presensi.kode_kelas = kelas.id_kelas where jadwal.kode_guru='$_SESSION[Id_User]'");
						if($querydosen == false){
