...
<tbody>
	...
	$querydosen = mysqli_query($konek, "select * from tagihan inner join kelas on tagihan.id_kelas = kelas.id_kelas inner join siswa on tagihan.id_siswa = siswa.NIS inner join bulan on tagihan.id_bulan = bulan.id_bulan_spp where siswa.nis = '$_SESSION[Id_User]' and tagihan.id_kelas = '$res1'");
	if ($querydosen == false) {
	...
	if ($test < 1) {
		...
		while ($tagihan = mysqli_fetch_array($querydosen)) {
			...
			$siswa = mysqli_query($konek, "SELECT nama_siswa FROM siswa WHERE id_siswa = '$tagihan[id_siswa]'");
